...
as kickplate jobs with tracking and visual status indicators.

v2.0 - Now uses "Kickplate -" prefixed stages to identify kickplate orders
...
# Stages we WANT to see (kickplate jobs in progress)
# Now using "Kickplate -" prefixed stages to identify kickplate orders
# This replaces Distribution Branch filtering and avoids stock issues
ACTIVE_STAGES = ['Kickplate - New', 'Kickplate - Processing', 'Kickplate - Job Complete', 'Kickplate - To Collect']

# Kickplate stage prefix - used to identify kickplate orders by stage name
...
# Orders are now identified as kickplate jobs by their "Kickplate -" prefixed stage
DISTRIBUTION_BRANCH_ID = None  # Disabled - using stage-based filtering instead
DISTRIBUTION_BRANCH_NAME = "Kickplate"  # For display only
...
st.set_page_config(
    page_title="Kickplate Board | Hardware Direct",
    page_icon="🏷️",
    layout="wide",
    initial_sidebar_state="collapsed"
)

# Hide sidebar completely (disabled when DEBUG_MODE is on)
if not DEBUG_MODE:
    st.markdown("""
    <style>
...
# Add logo to sidebar
try:
    add_logo(logo_path="Logos-01.jpg", text="Hardware Direct", subtitle="Kickplate Board")
except:
    add_logo(text="Hardware Direct", subtitle="Kickplate Board")
...
def inject_custom_css():
    ...
    <style>
        ...
        .tv-kpi-card.processing .tv-kpi-value {{
            color: {HDL_THEME['ok']};
        }}
        ...
        .tv-section.processing .tv-section-count {{
            color: {HDL_THEME['ok']};
        }}
...
def build_where_clause(excluded_stages: List[str]) -> str:
    ...
    Kickplate orders are now identified by their "Kickplate -" prefixed stage names.
...
def fetch_sales_orders(excluded_stages: tuple) -> pd.DataFrame:
    """
    Fetch all active sales orders filtered by Kickplate stages.

    Orders are identified as kickplate jobs by their "Kickplate -" prefixed stage names,
...
def process_orders_dataframe(orders: List[Dict]) -> pd.DataFrame:
    ...
    Filters orders by "Kickplate -" prefixed stage names instead of Distribution Branch.
    ...
    for order in orders:
        ...
        # Filter by Kickplate stage prefix instead of Distribution Branch
        # Only include orders whose stage starts with "Kickplate - "
        ...
        processed.append({
            ...
            "FirstName": get_field("FirstName") or "",
...
def render_job_card(job: pd.Series) -> str:
    ...
    # Stage badge - show shortened stage name (strip "Kickplate - " prefix for cleaner display)
    stage = job["Stage"] or "Unknown"
    display_stage = stage.replace(KICKPLATE_STAGE_PREFIX, "") if stage.startswith(KICKPLATE_STAGE_PREFIX) else stage
    stage_badge = f'<span class="badge badge-stage">{display_stage}</span>'

    # Kickplate count badge
    qty = int(job["QtyTotal"]) if job["QtyTotal"] else 0
    kp_badge = f'<span class="badge badge-size">{qty} KP</span>' if qty else ''
    ...
    contact_first = str(job["FirstName"] or "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
...
def render_currently_working_on(df: pd.DataFrame):
    """Render the 'Currently Working On' section for Kickplate - Processing stage jobs."""
    processing_df = df[df["Stage"] == "Kickplate - Processing"]
...
def render_to_collect(df: pd.DataFrame):
    ...
    to_collect_df = df[df["Stage"] == "Kickplate - To Collect"]
...
def render_board(df: pd.DataFrame):
    ...
    if df.empty:
        st.info("No active kickplate jobs found. Orders need a 'Kickplate -' prefixed stage to appear here.")
        return

    # Render "Currently Processing" section first - GREEN (Kickplate - Processing stage jobs)
    ...
    non_processing_df = df[(df["Stage"] != "Kickplate - Processing") & (df["Stage"] != "Kickplate - To Collect")]
...
def render_tv_header():
    ...
    header_html += '<h1 class="tv-title">Kickplate Board</h1>'
    header_html += '</div>'
    header_html += '<div>'
...
def render_tv_kpi_cards(df: pd.DataFrame):
    ...
    if df.empty:
        ...
        processing_count = int((df["Stage"] == "Kickplate - Processing").sum())
        to_collect_count = int((df["Stage"] == "Kickplate - To Collect").sum())
    ...
    <div class="tv-kpi-container">
        ...
        <div class="tv-kpi-card processing">
            <div class="tv-kpi-value">{processing_count}</div>
            <div class="tv-kpi-label">PROCESSING</div>
        </div>
...
def render_tv_job_card(job: pd.Series) -> str:
    ...
    contact_first = html.escape(str(job["FirstName"] or ""))

    # Kickplate count
    qty = int(job["QtyTotal"]) if job["QtyTotal"] else 0
    kp_str = str(qty) + " KP" if qty else ""
...
def render_tv_board(df: pd.DataFrame):
    ...
    # Get jobs currently being worked on (Kickplate - Processing stage)
    processing_df = df[df["Stage"] == "Kickplate - Processing"].sort_values("EstimatedDeliveryDate") if not df.empty else pd.DataFrame()

    # Get jobs ready for customer pickup (Kickplate - To Collect stage)
    to_collect_df = df[df["Stage"] == "Kickplate - To Collect"].sort_values("EstimatedDeliveryDate") if not df.empty else pd.DataFrame()

    # Get jobs in queue (due soon, not overdue, not processing, not to collect)
    if not df.empty:
        non_special = df[(df["Stage"] != "Kickplate - Processing") & (df["Stage"] != "Kickplate - To Collect")]
    ...
    processing_cards = render_tv_column_cards(processing_df, max_cards=6)
    ...
    processing_count = len(processing_df)
    ...
    <div class="tv-board">
        ...
        <div class="tv-section processing">
            <div class="tv-section-header">
                <span class="tv-section-title">PROCESSING</span>
                <span class="tv-section-count">{processing_count}</span>
            </div>
            <div class="tv-cards-column">
                {processing_cards}
            </div>
...
def render_tv_view(df: pd.DataFrame):
    ...
    Designed for wall-mounted TV displays.
...
def render_data_table(df: pd.DataFrame):
    ...
    st.download_button(
        label="Download CSV",
        data=csv,
        file_name=f"kickplate_jobs_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        mime="text/csv"
    )
...
def main():
    ...
    # All known stages including the new Kickplate-prefixed ones
    all_stages = [
        "Kickplate - New", "Kickplate - Processing", "Kickplate - Job Complete", "Kickplate - To Collect",  # Kickplate stages
        "New", "Processing", "Job Complete",  # Regular stages (non-kickplate)
    ...
    with tab_desktop:
        ...
        st.markdown(f"""
        <div class="info-box">
            <strong>Filter:</strong> Kickplate stages (Kickplate - New, Processing, Job Complete, To Collect) &nbsp;|&nbsp;
        ...
        with col1:
            # Default to only active kickplate stages
        ...
        with st.spinner("Loading kickplate data..."):
        ...
        <div style="text-align: center; color: #6C757D; font-size: 0.8rem; margin-top: 2rem; padding: 1rem;">
            HDL Kickplate Board v2.0 | Timezone: {TIMEZONE_DISPLAY} |
            Data cached for 5 minutes | Filtering by Kickplate stages
        </div>
        """, unsafe_allow_html=True)
